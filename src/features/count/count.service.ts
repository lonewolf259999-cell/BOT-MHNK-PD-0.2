...
export async function manualRecount(client: any, interaction: any): Promise<void> {
    return locks.count.run(async () => {
        ...
        // ไม่อ่าน clear C4:G ก่อน เพื่อป้องกันข้อมูลนับปัจจุบันหาย
        // ใช้ lock.count.run() เพื่อรอให้ processCountBatch ปัจจุบันเสร็จก่อน
        const rows = await sheetService.getValues(cfg.SPREADSHEET_ID, `${cfg.SHEET_NAME}!A:G`, 0) || [];
        // reset ค่าเก่าเฉพาะ column C-G (index 2-6)
