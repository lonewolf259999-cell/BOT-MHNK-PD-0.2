...
async function moveToOutDC(sid: string, sn: string, osn: string, row: string[], reason: string) {
    ...
    for (const col of ['B', 'D', 'E', 'F', 'G', 'H', 'J', 'K', 'M', 'O', 'P', 'Q', 'R', 'S', 'T', 'U']) await sheetService.clearValues(sid, `${sn}!${col}${nr}`).catch(() => {});
}
    ...
    client.on(Events.InteractionCreate, async (i: any) => {
        ...
        try {
            ...
            for (let idx = 2; idx < rows.length; idx++) {
                ...
                if (!mem) { await moveToOutDC(reg.spreadsheetId, reg.sheetName, reg.outSheetName, row, '30Day (ไม่พบใน DC)'); done.push(`แถว ${idx + 1} (${uid}): ย้ายออก`); continue; }
                if (CONFIG.EXEMPT_ROLES.some(r => mem.roles.cache.has(r))) { skip.push(`${mem.user.tag}: มี EXEMPT`); continue; }
                await moveToOutDC(reg.spreadsheetId, reg.sheetName, reg.outSheetName, row, '30Day');
                const toRemove = mem.roles.cache.filter(r => r.id !== mem.guild.id && !CONFIG.EXEMPT_ROLES.includes(r.id) && r.id !== CONFIG.THIRTY_DAY_ROLE_ID);
